...
function errorStatus(error: unknown) {
  ...
  if (error.message === "INVALID_JSON" || error.message === "INVALID_ROOM") return 400;
  return 500;
}
...
export function createRoomApi(
  ...
  return async (request: IncomingMessage, response: ServerResponse, path: string): Promise<boolean> => {
    if (path === "/api/rooms" && request.method === "GET") {
      ...
      json(response, 200, { rooms: await repository.listOpen() });
      return true;
    }
    ...
    if (!participant || participant.userId !== user.id) { json(response, 401, { error: "UNAUTHORIZED" }); return true; }
...
function publicError(error: unknown): string {
  const allowed = new Set(["FORBIDDEN", "LOCKED", "OCCUPIED", "ALREADY_SEATED", "NOT_FOUND", "PAYLOAD_TOO_LARGE", "INVALID_JSON", "INVALID_ROOM"]);
